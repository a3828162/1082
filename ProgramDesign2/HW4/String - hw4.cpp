...
string &string::operator=( const string &right )
{
   if( this != &right )
   {
       if (right.myData.mySize > myData.mySize&& right.myData.mySize > myData.myRes)
       {
           size_type newMyRes = myData.myRes * 3 / 2;
           if (newMyRes < (right.myData.mySize / 16) * 16 + 15)
               newMyRes = (right.myData.mySize / 16) * 16 + 15;

           value_type* str = new value_type[newMyRes]();
           for (int i = 0; i < right.myData.mySize; i++)
               str[i] = right.myData.myPtr()[i];
           if (myData.myRes > 15)
               delete[]myData.bx.ptr;
           myData.bx.ptr = str;
           myData.mySize = right.myData.mySize;
...
{
    if (newSize > myData.mySize&& newSize > myData.myRes)
    {
        size_type newMyRes = myData.myRes * 3 / 2;
    ...
    else if (newSize > myData.mySize&& myData.myRes > 15)
    {
        for (int i = 0; i < myData.mySize; i++)
            myData.bx.ptr[i] = myData.myPtr()[i];
        for (int i = myData.mySize; i < newSize; i++)
            myData.bx.ptr[i] = ch;
        myData.mySize = newSize;
    }
