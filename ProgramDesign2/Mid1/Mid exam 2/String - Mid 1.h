...
public:
   ...
   iterator insert( const_iterator where, const char ch )
   {
      ...
      {
		  int s = size() + 1;
		  int newRes = newRes = (s / 16) * 16 + 15;
		  if (myData.myRes * 1.5 > newRes)
			  newRes = myData.myRes * 1.5;
		  
		  if (s > myData.myRes)
		  {
			  pointer str = new value_type[newRes]();
			  string::iterator it1 = begin();
		  ...
		  {
			  pointer str = new value_type[myData.myRes]();
			  string::iterator it1 = begin();
			  for (int i = 0, j = 0; i < s; i++, ++it1, j++)
			  {
				  if (where == it1)
