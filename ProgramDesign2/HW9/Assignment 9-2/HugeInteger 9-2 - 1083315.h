...
#include "C:\Users\James\Desktop\YZU\程設作業\HW9\Vector - 1083315 - hw9.h"
#include "C:\Users\James\Desktop\YZU\程設作業\HW9\List - 1083315 - hw9.h"
#include "C:\Users\James\Desktop\YZU\程設作業\HW9\Deque - 1083315 - hw9.h"
...
bool HugeInteger< T >::operator<( HugeInteger< T > &right )
{
    if (this->integer.size() != right.integer.size())
        return this->integer.size() < right.integer.size();

    auto it1 = this->integer.rbegin(), it2 = right.integer.rbegin();
...
HugeInteger< T > HugeInteger< T >::operator-( HugeInteger< T > &op2 )
{
   ...
   auto it1 = op2.integer.begin();
   auto it2 = difference.integer.begin();
   auto it3 = difference.integer.begin();
   for (; it1 != op2.integer.end(); ++it1, ++it2, it3 = it2)
   {
       if (*it2 < *it1)
       {
           while (*(++it3) == 0)
   ...
   while (difference.integer.back() == 0 && difference.integer.size() > 1)
...
HugeInteger< T > HugeInteger< T >::operator*( HugeInteger< T > &op2 )
{
   ...
   auto it1 = this->integer.begin();
   auto it2 = op2.integer.begin();
   auto it3 = product.integer.begin();
   auto it4 = it3;

   for (; it1 != this->integer.end(); ++it1)
   {
       it3 = it4;
       for (it2 = op2.integer.begin(); it2 != op2.integer.end(); ++it2)
       {
           *(it3) += *(it1) * *(it2);
           ++it3;
       }
...
HugeInteger< T > HugeInteger< T >::operator/( HugeInteger< T > &op2 )
{
   ...
   unsigned int a = this->integer.size() - op2.integer.size();
   auto it1 = buffer.integer.begin();
   auto it2 = op2.integer.begin();
   for (size_t i = 0; i < a; i++, ++it1);
   for (; it1 != buffer.integer.end(); ++it1, ++it2)
       *(it1) = *(it2);

   it1 = buffer.integer.begin();
   ...
   quotient.integer.resize(n);

   auto it3 = quotient.integer.rbegin();

   for (; it3 != quotient.integer.rend(); ++it3)
   {
       while (buffer <= remainder)
       {
           HugeInteger<T> b(remainder - buffer);
           remainder = b;
           ++* it3;
