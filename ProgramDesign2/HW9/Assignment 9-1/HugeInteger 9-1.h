...
HugeInteger< T > HugeInteger< T >::operator-( HugeInteger< T > &op2 )
{
   ...
   auto it1 = op2.begin();
   auto it2 = difference.begin();
   auto it3 = difference.begin();
   for (; it1 != op2.end(); ++it1, ++it2, it3 = it2)
   {
       if (*it2 < *it1)
       {
           while (*(++it3) == 0)
   ...
   while (difference.back() == 0 && difference.size() > 1)
       difference.pop_back();
   /*auto it4 = difference.rbegin();
   for (; *(it4)==0&&it4 != difference.rend(); ++it4)
       difference.pop_back();*/

   //while (difference.back() == 0 && difference.size() > 0)
     //  difference.pop_back();
...
HugeInteger< T > HugeInteger< T >::operator*( HugeInteger< T > &op2 )
{
   ...
   it3 = product.begin();
   it4 = it3;
   for (++it4; it4 != product.end(); ++it3, ++it4)
       if (*(it3) > 9)
       {
           *(it4) += *(it3) / 10;
           *(it3) %= 10;
   ...
   while (product.back() == 0 && product.size() > 0)
       product.pop_back();
...
HugeInteger< T > HugeInteger< T >::operator/( HugeInteger< T > &op2 )
{
   ...
   unsigned int a = this->size() - op2.size();
   auto it1 = buffer.begin();
   auto it2 = op2.begin();
   for (size_t i = 0; i < a; i++, ++it1);
   for (; it1 != buffer.end(); ++it1, ++it2)
       *(it1) = *(it2);
   ...
   while (quotient.back() == 0 && quotient.size() > 1)
       quotient.pop_back();
       */
   return quotient;
} // end function operator/
