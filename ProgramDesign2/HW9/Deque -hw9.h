...
public:
      ...
      : myData()
   {
      ...
      for (size_t i = 0; i < (4 * right.myData.mapSize); i++)
      {
          if (right.myData.map[i / 4] != nullptr && myData.map[i / 4] == nullptr)
   ...
   deque& operator=( const deque &right )
   {
      if( &right != this ) // avoid self-assignment
      {
          ...
          {
              Ty** newMap = new Ty * [right.myData.mapSize]();

              for (size_t i = 0; i < (4 * right.myData.mapSize); i++)
              {
                  if (right.myData.map[i / 4] != nullptr && newMap[i / 4] == nullptr)
                      newMap[i / 4] = new Ty[4]();
                  if (right.myData.map[i / 4] != nullptr)
                      newMap[i / 4][i % 4] = right.myData.map[i / 4][i % 4];
              }
