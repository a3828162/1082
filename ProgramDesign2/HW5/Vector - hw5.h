...
public:
      ...
      : myData()
   {
       ...
       myData.myFirst = str;
       myData.myEnd = myData.myLast = str + count;
   }
   ...
   vector( const vector &right )
      : myData()
   {
       value_type s = right.myData.myLast - right.myData.myFirst;
       value_type c = right.myData.myEnd - right.myData.myFirst;
       ...
       if (right.myData.myFirst != nullptr) {
           for (value_type i = 0; i < s; i++)
               str[i] = *(right.myData.myFirst + i);
       }
       else {
