...
#include "Vector - 1083315 - hw7.h" // include definition of class template vector
#include "List - 1083315 - hw7.h"   // include definition of class template list
...
HugeInteger< T > HugeInteger< T >::operator-( HugeInteger< T > &op2 )
{
   ...
   int* a = new int[difference.integer.size()]();
   for (unsigned int i = 0; i < difference.integer.size(); i++, ++it2)
       a[i] = *it2;

   int j = 0;
   it1 = op2.integer.begin();
   it2 = difference.integer.begin();
   for (; it1 != op2.integer.end(); ++it1, ++it2, j++)
   {
       a[j] = *it2 - *it1;
   }
   ...
   j = 0;
   for (it2 = difference.integer.begin(); it2 != difference.integer.end(); ++it2, ++j)
   {
       *(it2) = a[j];
   }
...
HugeInteger< T > HugeInteger< T >::operator*(HugeInteger< T >& op2)
{
    ...
    int* a = new int[productSize]();
    unsigned int j = 0;
    for (unsigned int i = 0; it1!=integer.end(); ++i, ++it1)
    ...
    for (unsigned int i = 0; i < productSize; i++, ++it3)
        *it3 = a[i];
...
HugeInteger< T > HugeInteger< T >::operator/( HugeInteger< T > &op2 )
{
   ...
   auto it4 = remainder.integer.begin();

   unsigned int bufferSize = divisorSize + n;

   HugeInteger buffer(dividendSize);
   auto it5 = buffer.integer.begin();

   int* a = new int[dividendSize]();

   it2 = op2.integer.end();
   --it2;

   for (int i = divisorSize - 1; i >= 0; i--, --it2)
       a[n + i] = *it2;
   it2 = op2.integer.begin();
   unsigned int j = 0;
   for (; it5 != buffer.integer.end(); ++it5, j++)
       *it5 = a[j];
   it5 = buffer.integer.begin();
   ...
   for (; it3 != quotient.integer.begin(); --it3)
   {
       while (buffer <= remainder)
       {
           HugeInteger<T>c(remainder - buffer);
          remainder = c;
           ++* it3;
